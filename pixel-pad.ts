...
interface PixelPadInterface {
    wrapperEl: HTMLElement
    // 指针显示层
    canvasElCursor: HTMLCanvasElement
    ctxCursor: CanvasRenderingContext2D
    // 离屏绘制层
    canvasElOff: HTMLCanvasElement
    ctxOff: CanvasRenderingContext2D
    // 背景层
    canvasElBackground: HTMLCanvasElement
    ...
    // 配置
    option: Option // 用户配置
    optionOrigin: Option // 用户配置原始值，重置option使用
    optionDefault: Option
    preDrawBoxPoint: BoxPosition | null
    isDrawing: boolean
    drawInfoHistory: Array<Array<DrawInfo>>
    drawInfoHistoryFull: Array<Array<DrawInfo>>
    drawInfoHistoryForRedo: Array<Array<DrawInfo>>
    drawInfoHistoryFullForRedo: Array<Array<DrawInfo>>
    ...
    fillBoxInCanvasInOff(boxX: number, boxY: number, color: string): void

    fillBoxInCanvasByDrawInfo(drawInfo: Array<DrawInfo>): void

    getBoxXY(pixelX: number, pixelY: number): BoxPosition

    buildDrawInfoFull(): Array<DrawInfo>

    setUsingTool(toolName: ToolName): void

    undo(): void

    redo(): void

    load(drawInfo: Array<DrawInfo>): void

    view(drawInfo: Array<DrawInfo>): void

    save(): Array<DrawInfo>
}

export default class PixelPad implements PixelPadInterface {
    wrapperEl: HTMLElement
    // 指针显示层
    canvasElCursor: HTMLCanvasElement
    ctxCursor: CanvasRenderingContext2D
    ...
    preDrawBoxPoint: BoxPosition | null = null
    ...
    setPad(): void {
        const setCanvasSize = (canvasEl: HTMLCanvasElement) => {
    ...
    setAction(): void {
        const drawHandler = (e: { offsetX: any; offsetY: any }) => {
            const x = e.offsetX
            const y = e.offsetY
            ...
            if (this.preDrawBoxPoint === null) {
                ...
                this.drawInfoHistory[this.drawInfoHistory.length - 1].push(<DrawInfo>{x: boxX, y: boxY, color: this.option.drawColor})//擦除的位置color为null
    ...
    setCursor(): void {
        let curseImage: CanvasImageSource
        {
    ...
    fillBoxInCanvasInOff(boxX: number, boxY: number, color: string | null): void {
    ...
    //设置绘画颜色
    setColor(colorHex: string | null): void {
    ...
    // 获取最终绘画像素信息
    save(): Array<DrawInfo> {
        const finallyDrawInfo: Array<DrawInfo> = []
